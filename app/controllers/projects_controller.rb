  ...
  def show
    ...
    when :scenarios
      @scenarios_grid = ScenarioGrid.new do |scope|
        if @current_label
          scope.joins(:labels).where(project: @project, issue: @issue, labels: { id: @current_label.id }).page(params[:page]).per(20)
        elsif @no_label
          ...
          scope.where(project: @project, issue: @issue).where.not(id: scenario_with_labels_ids).page(params[:page]).per(20)
        else
          scope.where(project: @project, issue: @issue).page(params[:page]).per(20)
        end
      end
